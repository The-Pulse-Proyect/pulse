...
export function useCava(): AudioNodes | null {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Acceder a los elementos internos de Howler de manera segura
      const howl = currentSound as any;
      const audioElement = howl._sounds?.[0]?._node as HTMLMediaElement;
      
      if (!audioElement) {
        console.error("No se pudo obtener el elemento de audio");
        return;
      }
