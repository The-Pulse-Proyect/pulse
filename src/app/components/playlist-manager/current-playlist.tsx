...
export function CurrentPlaylist() {
  ...
  const handleOpenFile = async () => {
    try {
      const filePaths = await window.tauriAPI.openFile();
      
      if (filePaths && Array.isArray(filePaths) && filePaths.length > 0) {
        const songMetadata = await window.tauriAPI.processMetadata(filePaths);
        if (songMetadata && Array.isArray(songMetadata)) {
          // Agregar un ID único a cada canción si no lo tiene
          const songsWithIds = songMetadata.map(song => ({
            ...song,
            id: song.id || `song-${Date.now()}-${Math.random()}`
          }));
          addSongsToPlaylist(songsWithIds);
        }
      }
    } catch (error) {
